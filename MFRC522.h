...
//	enum for IRQ`s
...
#elif defined __STM32__
	MFRC522();
	MFRC522(GPIO_TypeDef *nRST_Port, uint32_t nRST_Pin);
	MFRC522(GPIO_TypeDef *nCS_Port, uint32_t nCS_Pin, GPIO_TypeDef *nRST_Port,
			uint32_t nRST_Pin);
#endif
...
#elif defined __STM32__
	void PCD_Init(GPIO_TypeDef *nRST_Port, uint32_t nRST_Pin);
	void PCD_Init(GPIO_TypeDef *nCS_Port, uint32_t nCS_Pin,
	...
	byte *backLen, byte *validBits = nullptr, byte rxAlign = 0, bool checkCRC =
			false);
	...
	char
#endif
	* GetStatusCodeName(StatusCode code);
