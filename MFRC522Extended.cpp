...
MFRC522::StatusCode MFRC522Extended::PICC_RequestATS(Ats *ats)
{
	...
	result = PCD_TransceiveData(bufferATS, 4, bufferATS, &bufferSize, NULL, 0,
			true);
	if (result != STATUS_OK)
	{
		PICC_HaltA();
...
MFRC522::StatusCode MFRC522Extended::PICC_PPS()
{
	...
	// Transmit the buffer and receive the response, validate CRC_A.
	result = PCD_TransceiveData(ppsBuffer, 4, ppsBuffer, &ppsBufferSize, NULL,
			0, true);
	if (result == STATUS_OK)
	{
		// Enable CRC for T=CL
		...
		)
{
	...
	ppsBuffer[2] = (((sendBitRate & 0x03) << 2) | (receiveBitRate & 0x03))
			& 0xE7;
	...
	// Transmit the buffer and receive the response, validate CRC_A.
	result = PCD_TransceiveData(ppsBuffer, 5, ppsBuffer, &ppsBufferSize, NULL,
			0, true);
	if (result == STATUS_OK)
	{
...
result = PCD_TransceiveData(outBuffer, outBufferOffset, inBuffer,
		&inBufferSize);
if (result != STATUS_OK)
{
	return result;
...
if ((rxModeReg & 0x80) != 0x80)
{
	...
	MFRC522::StatusCode status = PCD_CalculateCRC(inBuffer, inBufferSize - 2,
			controlBuffer);
	if (status != STATUS_OK)
	{
		return status;
...
MFRC522::StatusCode MFRC522Extended::TCL_TransceiveRBlock(TagInfo *tag,
	bool ack, byte *backData, byte *backLen)
{
MFRC522::StatusCode result;
