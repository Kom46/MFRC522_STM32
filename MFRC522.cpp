...
#include "main.h"
#include <string.h>
#ifndef RC522_SPI_HANDLE_POINTER
#error "Define spi handle pointer for your RC522 in main.h in format #define RC522_SPI_HANDLE_POINtER <(&your_spi_handle)>"
#endif
#endif
#include "MFRC522.h"
//#include <cstring>
...
#elif defined __STM32__
MFRC522::MFRC522() :
		MFRC522(RC522_nCS_GPIO_Port, RC522_nCS_Pin, RC522_nRST_GPIO_Port,
		RC522_nRST_Pin)
{
}
;
MFRC522::MFRC522(GPIO_TypeDef *nRST_Port, uint32_t nRST_Pin) :
		MFRC522(RC522_nCS_GPIO_Port, RC522_nCS_Pin, nRST_Port, nRST_Pin)
{
}
;
MFRC522::MFRC522(GPIO_TypeDef *nCS_Port, uint32_t nCS_Pin,
		GPIO_TypeDef *nRST_Port, uint32_t nRST_Pin)
{
	_nCS_Port = nCS_Port;
	_nCS_Pin = nCS_Pin;
	_nRST_Port = nRST_Port;
	_nRST_Pin = nRST_Pin;
}
#endif
/////////////////////////////////////////////////////////////////////////////////////
...
#elif defined __STM32__
//	HAL_Delay(1);
	HAL_GPIO_WritePin(_nCS_Port, (uint16_t) _nCS_Pin, GPIO_PIN_RESET);
	uint8_t buff[2] =
	{ reg, value };
	HAL_SPI_Transmit(RC522_SPI_HANDLE_POINTER, buff, 2, 50);
//#if (DEBUG == 1)
//	uint8_t val = PCD_ReadRegister(reg);
//#endif
...
//	HAL_Delay(1);
	HAL_GPIO_WritePin(_nCS_Port, (uint16_t) _nCS_Pin, GPIO_PIN_RESET);
	uint8_t * buff = new uint8_t[count + 1];
	buff[0] = reg;
	memcpy(&buff[1], values, count);
	HAL_SPI_Transmit(RC522_SPI_HANDLE_POINTER, buff, count + 1, 500);
	delete[] buff;
	HAL_GPIO_WritePin(_nCS_Port, _nCS_Pin, GPIO_PIN_SET);
//	HAL_Delay(1);
#endif
...
//	HAL_Delay(1);
	HAL_GPIO_WritePin(_nCS_Port, (uint16_t) _nCS_Pin, GPIO_PIN_RESET);
//	HAL_SPI_Receive_DMA(RC522_SPI_HANDLE_POINTER, &value, 1);
...
//	address |= reg;
	...
	uint8_t rx_buff[2] =
	{ 0 };
//	HAL_SPI_Transmit(RC522_SPI_HANDLE_POINTER, &address, 1, 500);
//	uint8_t zero = 0;
//	HAL_SPI_Receive_DMA(RC522_SPI_HANDLE_POINTER, &value, 1);
//	HAL_SPI_TransmitReceive_DMA(RC522_SPI_HANDLE_POINTER, &zero, &value, 1);
//	extern bool spi_receive_is_end;
//	HAL_SPI_Transmit(RC522_SPI_HANDLE_POINTER, &zero, 1, 500);
//	HAL_Delay(1);
	HAL_SPI_TransmitReceive(RC522_SPI_HANDLE_POINTER, tx_buff, rx_buff, 2, 500);
	HAL_GPIO_WritePin(_nCS_Port, _nCS_Pin, GPIO_PIN_SET);
//	HAL_Delay(1);
#endif
	return rx_buff[1];
...
//	HAL_Delay(1);
	uint8_t *tx_buff = new uint8_t[count + 1];
	uint8_t *rx_buff = new uint8_t[count + 1];
	memset(tx_buff, address, count);
	tx_buff[count] = 0;
	memset(rx_buff, 0, count);
	HAL_SPI_TransmitReceive(RC522_SPI_HANDLE_POINTER, tx_buff, rx_buff,
			count + 1, 500);
#endif
#if defined __ARDUINO__
...
//	HAL_SPI_Transmit(RC522_SPI_HANDLE_POINTER, &address, 1, 500);
	...
	{		// Only update bit positions rxAlign..7 in values[0]
		...
		// Apply mask to both current value of values[0] and the new data in value.
			...
			index++;
#elif defined __STM32__
//		byte value = rx_buff[1];
//		uint8_t tx_buff[2] = {address, address};
//		uint8_t rx_buff[2] = {0};
//		HAL_SPI_TransmitReceive(RC522_SPI_HANDLE_POINTER, tx_buff, rx_buff, 2,
//				500);
//		byte value = rx_buff[1];
#endif
	}
...
#elif defined __STM32__
//		uint8_t tx_buff[count] = {0};
//		uint8_t rx_buff[count] = {0};
//		uint8_t *tx_buff = new uint8_t[count];
//		uint8_t * rx_buff = new uint8_t[count];
//		memset(tx_buff, address, count - 1);
//		HAL_SPI_TransmitReceive(RC522_SPI_HANDLE_POINTER, tx_buff,
//				rx_buff, count, 500);
//		memcpy(&values[1], rx_buff, count);
//		delete[] tx_buff;
//		delete[] rx_buff;
		index = count;
		break;
...
//	uint8_t tx_buff[2] = {0};
//	uint8_t rx_buff[2] = {0};
//	HAL_SPI_TransmitReceive(RC522_SPI_HANDLE_POINTER, tx_buff, rx_buff, 1,
//			500);
//	values[index] = rx_buff[1];
	int start = 0;
	if (rxAlign)
	{
		byte mask = (0xFF << rxAlign) & 0xFF;
		values[0] = (values[0] & ~mask) | (rx_buff[1] & mask);
		start++;
	}
	memcpy(&values[start], &rx_buff[start + 1], count - start);
	HAL_GPIO_WritePin(_nCS_Port, _nCS_Pin, GPIO_PIN_SET);
#endif
} // End PCD_ReadRegister()
...
#elif defined __STM32__
	PCD_Init(RC522_nCS_GPIO_Port, RC522_nCS_Pin, nRST_Port, nRST_Pin);
#endif
} // End PCD_Init()
...
void MFRC522::PCD_Init(
		...
		byte resetPowerDownPin///< Arduino pin connected to MFRC522's reset and power down input (Pin 6, NRSTPD, active low)
#elif defined __STM32__
		GPIO_TypeDef *nCS_Port, uint32_t nCS_Pin, GPIO_TypeDef *nRST_Port,
		uint32_t nRST_Pin
#endif
		)
{
...
#elif defined __STM32__
	byte waitIRq = RxIRq | IdleIRq;		// RxIRq and IdleIRq
#endif
		...
		)
{
...
#if (DEBUG == 1)
		volatile uint8_t errReg = PCD_ReadRegister(ErrorReg);
		volatile uint8_t fifoLvlReg = PCD_ReadRegister(FIFOLevelReg);
		volatile uint8_t controlReg = PCD_ReadRegister(ControlReg);
		volatile uint8_t status1Reg = PCD_ReadRegister(Status1Reg);
		volatile uint8_t status2Reg = PCD_ReadRegister(Status2Reg);
		volatile uint8_t commandReg = PCD_ReadRegister(CommandReg);
#endif
		if (n & waitIRq)
		{			// One of the interrupts that signal success has been set.
			break;
	...
	} while ((timeout = (timeout >= HAL_GetTick()) ? timeout : 0));
	if (!timeout)
		return STATUS_TIMEOUT;
#endif
...
char
#endif
* MFRC522::GetStatusCodeName(MFRC522::StatusCode code ///< One of the StatusCode enums.
		)
{
#if defined __ARDUINO__
	switch (code)
...
// Then you can write to sector 0 without authenticating
	...
	byte received;
	MFRC522::StatusCode status = PCD_TransceiveData(&cmd, (byte) 1, response,
			&received, &validBits, (byte) 0, false); // 40
	if (status != STATUS_OK)
	{
		if (logErrors)
		{
...
// Authenticate for reading
	...
	MFRC522::StatusCode status = PCD_Authenticate(
...
bool MFRC522::MIFARE_UnbrickUidSector(bool logErrors)
{
	...
	byte block0_buffer[] =
	{ 0x01, 0x02, 0x03, 0x04, 0x04, 0x08, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00 };

// Write modified block 0 back to card
	MFRC522::StatusCode status = MIFARE_Write((byte) 0, block0_buffer,
			(byte) 16);
	if (status != STATUS_OK)
	{
		if (logErrors)
